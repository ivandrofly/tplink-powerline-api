...
**Every call is a form post.** Endpoints look like `admin/wireless?form=wireless_2g` with `operation=load|read|write|insert` and other fields as `application/x-www-form-urlencoded` parameters via `RestRequest.AddParameter(..., ParameterType.GetOrPost)`. Nothing is sent as a JSON body, so `JsonPropertyName` attributes only affect deserialization. When writing settings back (see `ChangeWireless2GStatusAsync`), the client reads the current model, mutates it, then reflects over its properties to emit each one as a form field using the `JsonPropertyName` name.
