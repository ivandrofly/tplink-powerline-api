...
public class DisplayConnectedCommand : ICommand
{
    ...
    {
        Console.WriteLine();
        Console.WriteLine("Executing display command");
        var result = await powerLine.GetClientsAsync();
        if (!result.Success || result.Data == null)
        {
            Console.WriteLine("The adapter rejected the request (make sure its web manager is not open in a browser).");
            return;
        }

        if (result.Data.Count == 0)
        {
            Console.WriteLine("No wireless clients connected.");
            return;
        }

        Console.WriteLine();
        Console.WriteLine($"{"Name",-30}|{"IP",-30}|Mac");
        foreach (var client in result.Data.OrderBy(c => LastOctet(c.IP)))
        {
            Console.WriteLine($"{client.DeviceName,-30}|{client.IP,-30}|{client.Mac?.Replace("-", ":")}");
        }
    }
