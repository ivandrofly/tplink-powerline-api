...
public class TpLinkClient : ITpLinkClient
{
    ...
    public const int DiscoveryPort = UdpTpLinkDiscovery.Port;

    /// <summary>Local UDP port the discovery reply is received on (open it in the firewall).</summary>
    public const int DiscoveryListenPort = UdpTpLinkDiscovery.ListenPort;

    /// <summary>How long <see cref="DiscoveryAsync()"/> waits for an adapter to answer.</summary>
    public static readonly TimeSpan DefaultDiscoveryTimeout = UdpTpLinkDiscovery.DefaultTimeout;
    ...
    /// <summary>
    /// The guest forms return the same shape for both bands with a <c>guest_2g_</c> / <c>guest_5g_</c> prefix on
    /// every field, so one <see cref="GuestNetwork"/> model is read with a per-band naming policy. The envelope's
    /// own properties carry explicit names and are unaffected.
    /// </summary>
    internal static readonly JsonSerializerOptions Guest2GJsonOptions = new(JsonOptions)
    {
        PropertyNamingPolicy = new PrefixedSnakeCaseNamingPolicy("guest_2g_"),
    };

    internal static readonly JsonSerializerOptions Guest5GJsonOptions = new(JsonOptions)
    {
        PropertyNamingPolicy = new PrefixedSnakeCaseNamingPolicy("guest_5g_"),
    };
    ...
    public TpLinkClient(EndpointAuth apiConnection, TimeSpan requestTimeout)
    {
        ...
        // everything below replays what the browser sends after login; see docs/device-notes.md
    ...
    /// <summary>
    /// Create a client from <paramref name="options"/>, discovering the adapter on the LAN with a UDP broadcast
    /// when <see cref="TpLinkOptions.Endpoint"/> is empty.
    /// </summary>
    ...
    public static Task<TpLinkClient> CreateAsync(TpLinkOptions options, CancellationToken cancellationToken = default) =>
        CreateAsync(options, UdpTpLinkDiscovery.Instance, cancellationToken);

    /// <summary>
    /// Create a client from <paramref name="options"/>, asking <paramref name="discovery"/> for the adapter's
    /// address when <see cref="TpLinkOptions.Endpoint"/> is empty.
    /// </summary>
    /// <exception cref="InvalidOperationException">Login or password is not configured.</exception>
    public static async Task<TpLinkClient> CreateAsync(TpLinkOptions options, ITpLinkDiscovery discovery, CancellationToken cancellationToken = default)
    {
        ...
        {
            var ip = await discovery.DiscoverAsync(options.DiscoveryTimeout, cancellationToken).ConfigureAwait(false);
            endpoint = $"http://{ip}";
        }
    ...
    private async Task<T> SendAsync<T>(RestRequest request, CancellationToken cancellationToken, JsonSerializerOptions? options = null) where T : class
    {
        ...
        return JsonSerializer.Deserialize<T>(response.Content, options ?? JsonOptions)
    ...
    /// (<c>RestRequest.AddObject</c> throws for this shape, hence the explicit flattening.)
    /// </summary>
    internal static void AddFormFields<T>(RestRequest request, T model)
    {
        ...
        foreach (var property in document.RootElement.EnumerateObject())
        {
            ...
            // GetString() is null only for a Null token, which was skipped above
    ...
    /// Posting hard-coded values for fields the device already reports silently resets them (docs/device-notes.md).
    /// </summary>
    ...
    public Task<TpLinkResponse<WifiMove>> SetWifiMoveAsync(bool enabled, CancellationToken cancellationToken = default)
    {
        ...
        // known not to take effect even though the request matches the browser's; see docs/device-notes.md
    ...
    /// <inheritdoc />
    public Task<TpLinkResponse<GuestNetwork>> GetGuest2GAsync(CancellationToken cancellationToken = default)
    {
        var req = NewFormRequest("admin/guest", "guest_2g", "read");
        return SendAsync<TpLinkResponse<GuestNetwork>>(req, cancellationToken, Guest2GJsonOptions);
    }

    /// <inheritdoc />
    public Task<TpLinkResponse<GuestNetwork>> GetGuest5GAsync(CancellationToken cancellationToken = default)
    {
        var req = NewFormRequest("admin/guest", "guest_5g", "read");
        return SendAsync<TpLinkResponse<GuestNetwork>>(req, cancellationToken, Guest5GJsonOptions);
    }
    ...
    public Task<TpLinkResponse<ICollection<WifiSchedule>>> AddNewWifiScheduleAsync(WifiSchedule wifiSchedule, CancellationToken cancellationToken = default)
    {
        ...
        req.AddParameter("index", "0", ParameterType.GetOrPost); // see docs/device-notes.md on rule ordering
    ...
    public static Task<string> DiscoveryAsync(TimeSpan timeout, CancellationToken cancellationToken = default) =>
        UdpTpLinkDiscovery.Instance.DiscoverAsync(timeout, cancellationToken);
